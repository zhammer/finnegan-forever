...
def current_passage_offset(scroll_size, passage_size, window_length, seconds):
    ...
    `window_length` seconds. We assume that after final passage in the scroll is read, the scroll is
    ...
    >>> current_passage_offset(scroll_size=270, passage_size=40, window_length=4, seconds=0)
    0

    # A second elapses. Since a full 4 second window has not yet elapsed, we are still at offset 0.
    >>> current_passage_offset(scroll_size=270, passage_size=40, window_length=4, seconds=1)
    0

    # Four seconds have elapsed. We are now reading from the second passage, at character 40.
    >>> current_passage_offset(scroll_size=270, passage_size=40, window_length=4, seconds=4)
    40
    ...
    >>> current_passage_offset(scroll_size=270, passage_size=40, window_length=4, seconds=24)
    240

    # Another four seconds elapse. We are back at the beginning of the scroll!
    >>> current_passage_offset(scroll_size=270, passage_size=40, window_length=4, seconds=28)
    0
    """
    num_passages_in_scroll = math.ceil(scroll_size / passage_size)
    windows_elapsed = seconds // window_length
